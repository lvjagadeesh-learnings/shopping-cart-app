...
5. **Create or update the PR**:
   - Check for an existing PR: `gh pr view --json url,number` (or `gh pr list --head <branch>`).
   - If none exists: `gh pr create --base main --title "<same as commit summary>" --body "<commit body, or a short bullet summary of the diff>"`.
   - If one exists: nothing further to do — the push above already updated it; just report its URL/number back to the user.
   - If `gh auth status` fails, tell the user to run `gh auth login` and stop.

6. **Report**: Summarize branch name, commit hash(es), whether a rebase/conflict occurred, push result, and the PR URL/number.
