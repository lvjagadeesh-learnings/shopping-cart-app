...
const ALL_STATUSES = ['Placed', 'Paid', 'Preparing', 'Shipped', 'OutForDelivery', 'Delivered', 'Cancelled'];
export function AdminOrdersPage() {
  ...
  return <div>
      ...
      <ul className="divide-y divide-gray-200 rounded border border-gray-200 bg-white">
        {orders?.map(order => <li key={order.id} className="flex items-center gap-4 p-4">
            ...
            <select value={order.status} onChange={e => updateStatus.mutate({
          orderId: order.id,
          payload: {
