...
import type { UpsertProductRequest } from '../api/types';
...
export function AdminProductsPage() {
  ...
  const { data: products, isLoading } = useProducts({ page: 1, pageSize: 50 });
  ...
  const { data: stock } = useAdminStockLevel(stockProductId ?? undefined);
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (editingId) {
      updateProduct.mutate({ productId: editingId, payload: form }, { onSuccess: resetForm });
    } else {
      createProduct.mutate(form, { onSuccess: resetForm });
    }
  }
      ...
      <form onSubmit={handleSubmit} className="mb-8 grid grid-cols-1 gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2">
        ...
        <div className="col-span-full flex gap-3">
          <button
            ...
            className="bg-brand-500 hover:bg-brand-600 rounded px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
          >
          ...
          <div className="w-80 rounded bg-white p-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() =>
                  adjustStock.mutate(
                    { productId: stockProductId, payload: { quantityOnHand: stockValue } },
