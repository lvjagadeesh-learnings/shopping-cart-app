  ...
  return <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
      <div className="lg:col-span-2">
        ...
        <ul className="divide-y divide-gray-200 rounded border border-gray-200 bg-white">
          {cart.items.map(item => <li key={item.id} className="flex items-center gap-4 p-4">
            ...
            updateItem.mutate({
              ...
              <button onClick={() => removeItem.mutate(item.productId)} className="text-sm text-red-500 hover:underline">
                Remove
              </button>
            </li>)}
        </ul>
      ...
      <div className="h-fit rounded border border-gray-200 bg-white p-4">
        ...
        <button onClick={() => navigate('/checkout')} className="bg-brand-500 hover:bg-brand-600 mt-4 w-full rounded py-2 font-medium text-white">
          Checkout
        </button>
      </div>
    </div>;
