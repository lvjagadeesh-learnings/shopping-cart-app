...
export function ProductDetailPage() {
  ...
  const { data: relatedProducts, isLoading: isLoadingRelated } = useRelatedProducts(product?.id);
    ...
    <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            {images.map((image, index) => (
              <button
                ...
                className={`h-16 w-16 overflow-hidden rounded border ${
                  index === activeImage ? 'border-brand-500' : 'border-gray-200'
                }`}
              >
      ...
      <div>
        ...
        <button
          ...
          className="bg-brand-500 hover:bg-brand-600 mt-4 rounded px-6 py-2 font-medium text-white disabled:opacity-50"
        >
